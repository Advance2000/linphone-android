...
public class EditContactFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		view = inflater.inflate(org.linphone.R.layout.edit_contact, container, false);
		
		TextView cancel = (TextView) view.findViewById(org.linphone.R.id.cancel);
		...
		ok.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (isNewContact) {
					if (getResources().getBoolean(org.linphone.R.bool.forbid_empty_new_contact_in_editor)) {
				...
				if(LinphoneActivity.instance().getResources().getBoolean(org.linphone.R.bool.isTablet))
					ContactsFragment.instance().invalidate();
			}
		});
		
		lastName = (EditText) view.findViewById(org.linphone.R.id.contactLastName);
		...
		firstName = (EditText) view.findViewById(org.linphone.R.id.contactFirstName);
		...
		initNumbersFields((TableLayout) view.findViewById(org.linphone.R.id.controls), contact);
	...
	private void initNumbersFields(final TableLayout controls, final Contact contact) {
		...
		if (!getResources().getBoolean(org.linphone.R.bool.hide_phone_numbers_in_editor)) {
			addEmptyRowToAllowNewNumberOrAddress(controls, false);
		}
		
		if (!getResources().getBoolean(org.linphone.R.bool.hide_sip_addresses_in_editor)) {
	...
	private View displayNumberOrAddress(final TableLayout controls, String numberOrAddress, boolean forceAddNumber) {
		...
		if ((getResources().getBoolean(org.linphone.R.bool.hide_phone_numbers_in_editor) && !isSip) || (getResources().getBoolean(org.linphone.R.bool.hide_sip_addresses_in_editor) && isSip)) {
			if (forceAddNumber)
		...
		final View view = inflater.inflate(org.linphone.R.layout.contact_edit_row, null);
		
		final EditText noa = (EditText) view.findViewById(org.linphone.R.id.numoraddr);
	...
	private void addEmptyRowToAllowNewNumberOrAddress(final TableLayout controls, final boolean isSip) {
		final View view = inflater.inflate(org.linphone.R.layout.contact_add_row, null);
		
		final NewOrUpdatedNumberOrAddress nounoa = new NewOrUpdatedNumberOrAddress(isSip);
		
		final EditText noa = (EditText) view.findViewById(org.linphone.R.id.numoraddr);
		numbersAndAddresses.add(nounoa);
		noa.setHint(isSip ? getString(org.linphone.R.string.sip_address) : getString(org.linphone.R.string.phone_number));
		noa.setInputType(isSip ? InputType.TYPE_TEXT_VARIATION_EMAIL_ADDRESS : InputType.TYPE_CLASS_PHONE);
		noa.requestFocus();
		noa.addTextChangedListener(new TextWatcher() {
			@Override
		...
		final ImageView add = (ImageView) view.findViewById(org.linphone.R.id.add);
		add.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// Add a line, and change add button for a delete button
				add.setImageResource(org.linphone.R.drawable.list_delete);
				...
				if (isSipAddress) {
					...
					if(!newNumberOrAddress.contains("@")) {
						...
						if(lpc != null){
							...
							newNumberOrAddress = newNumberOrAddress + "@" + getResources().getString(org.linphone.R.string.default_domain);
						}
					}
					...
					ops.add(ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI)
						...
						.withValue(ContactsContract.CommonDataKinds.Phone.LABEL, getString(org.linphone.R.string.addressbook_label))
				        .build()
				    );
				}
			} else {
