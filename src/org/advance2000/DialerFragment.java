...
*/
import org.advance2000.ui.CallButton;
import org.advance2000.ui.EraseButton;
import org.linphone.core.LinphoneCore;
import org.linphone.mediastream.Log;
import org.advance2000.ui.AddressAware;
...
public class DialerFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, 
		...
		instance = this;
        View view = inflater.inflate(org.linphone.R.layout.dialer, container, false);
		
		mAddress = (AddressText) view.findViewById(org.linphone.R.id.Adress);
		mAddress.setDialerFragment(this);
		
		EraseButton erase = (EraseButton) view.findViewById(org.linphone.R.id.Erase);
		erase.setAddressWidget(mAddress);
		
		mCall = (CallButton) view.findViewById(org.linphone.R.id.Call);
		mCall.setAddressWidget(mAddress);
		if (LinphoneActivity.isInstanciated() && LinphoneManager.getLc().getCallsNb() > 0) {
			if (isCallTransferOngoing) {
				mCall.setImageResource(org.linphone.R.drawable.transfer_call);
			} else {
				mCall.setImageResource(org.linphone.R.drawable.add_call);
			}
		} else {
			mCall.setImageResource(org.linphone.R.drawable.call);
		}
		
		AddressAware numpad = (AddressAware) view.findViewById(org.linphone.R.id.Dialer);
		if (numpad != null) {
			numpad.setAddressWidget(mAddress);
		}
		
		mAddContact = (ImageView) view.findViewById(org.linphone.R.id.addContact);
	...
	public void resetLayout(boolean callTransfer) {
		...
		if (lc.getCallsNb() > 0) {
			...
			mAddContact.setImageResource(org.linphone.R.drawable.cancel);
			mAddContact.setOnClickListener(cancelListener);
		} else {
			mCall.setImageResource(org.linphone.R.drawable.call);
			mAddContact.setEnabled(true);
			mAddContact.setImageResource(org.linphone.R.drawable.add_contact);
