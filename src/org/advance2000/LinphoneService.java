...
//	private boolean mTestDelayElapsed; // add a timer for testing
	...
	public void onCreate() {
		...
		mNotificationTitle = getString(org.linphone.R.string.service_name);

		// Needed in order for the two next calls to succeed, libraries must have been loaded first
		LinphoneCoreFactory.instance().setLogCollectionPath(getFilesDir().getAbsolutePath());
		LinphoneCoreFactory.instance().enableLogCollection(!(getResources().getBoolean(org.linphone.R.bool.disable_every_log)));
		...
		Bitmap bm = null;
		try {
			bm = BitmapFactory.decodeResource(getResources(), org.linphone.R.drawable.logo_linphone_57x57);
		} catch (Exception e) {
		}
		mNotif = Compatibility.createNotification(this, mNotificationTitle, "", org.linphone.R.drawable.status_level, IC_LEVEL_OFFLINE, bm, mNotifContentIntent, true, notifcationsPriority);
		...
		LinphoneManager.getLc().addListener(mListener = new LinphoneCoreListenerBase(){
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				...
				if (state == State.StreamsRunning) {
					...
					if (getResources().getBoolean(org.linphone.R.bool.enable_call_notification))
			...
			public void globalState(LinphoneCore lc,LinphoneCore.GlobalState state, String message) {
				if (state == GlobalState.GlobalOn) {
					sendNotification(IC_LEVEL_OFFLINE, org.linphone.R.string.notification_started);
				}
			}
...
//				}
				if (!mDisableRegistrationStatus) {
					if (state == RegistrationState.RegistrationOk && LinphoneManager.getLc().getDefaultProxyConfig() != null && LinphoneManager.getLc().getDefaultProxyConfig().isRegistered()) {
						sendNotification(IC_LEVEL_ORANGE, org.linphone.R.string.notification_registered);
					}
					...
					if ((state == RegistrationState.RegistrationFailed || state == RegistrationState.RegistrationCleared) && (LinphoneManager.getLc().getDefaultProxyConfig() == null || !LinphoneManager.getLc().getDefaultProxyConfig().isRegistered())) {
						sendNotification(IC_LEVEL_OFFLINE, org.linphone.R.string.notification_register_failure);
					}
	...
	private synchronized void setIncallIcon(IncallIconState state) {
		...
		case IDLE:
			...
			return;
		case INCALL:
			inconId = org.linphone.R.drawable.conf_unhook;
			notificationTextId = org.linphone.R.string.incall_notif_active;
			break;
		case PAUSE:
			inconId = org.linphone.R.drawable.conf_status_paused;
			notificationTextId = org.linphone.R.string.incall_notif_paused;
			break;
		case VIDEO:
			inconId = org.linphone.R.drawable.conf_video;
			notificationTextId = org.linphone.R.string.incall_notif_video;
			break;	
		default:
		...
		try {
			...
			bm = BitmapFactory.decodeResource(getResources(), org.linphone.R.drawable.unknown_small);
		}
	...
	public synchronized void sendNotification(int level, int textId) {
		...
		mNotif = Compatibility.createNotification(this, mNotificationTitle, text, org.linphone.R.drawable.status_level, level, bm, mNotifContentIntent, true,notifcationsPriority);
	...
	public void onTaskRemoved(Intent rootIntent) {
		if (getResources().getBoolean(org.linphone.R.bool.kill_service_with_task_manager)) {
