...
import org.advance2000.ui.AvatarWithShadow;
import org.advance2000.ui.Numpad;
...
public class InCallActivity extends FragmentActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(org.linphone.R.layout.incall);
        
        isTransferAllowed = getApplicationContext().getResources().getBoolean(org.linphone.R.bool.allow_transfers);
        showCallListInVideo = getApplicationContext().getResources().getBoolean(org.linphone.R.bool.show_current_calls_above_video);
        ...
        isAnimationDisabled = getApplicationContext().getResources().getBoolean(org.linphone.R.bool.disable_animations) || !LinphonePreferences.instance().areAnimationsEnabled();
...
//        			}
        ...
        if (findViewById(org.linphone.R.id.fragmentContainer) != null) {
            ...
            getSupportFragmentManager().beginTransaction().add(org.linphone.R.id.fragmentContainer, callFragment).commitAllowingStateLoss();
	...
	private boolean isTablet() {
		return getResources().getBoolean(org.linphone.R.bool.isTablet);
	}
	
	private void initUI() {
		inflater = LayoutInflater.from(this);
		container = (ViewGroup) findViewById(org.linphone.R.id.topLayout);
        callsList = (TableLayout) findViewById(org.linphone.R.id.calls);
        if (!showCallListInVideo) {
        	callsList.setVisibility(View.GONE);
        	
        }
        
		video = (TextView) findViewById(org.linphone.R.id.video);
		video.setOnClickListener(this);
		video.setEnabled(false);
		micro = (TextView) findViewById(org.linphone.R.id.micro);
		micro.setOnClickListener(this);
//		micro.setEnabled(false);
		speaker = (TextView) findViewById(org.linphone.R.id.speaker);
		speaker.setOnClickListener(this);
		if(isTablet()){
			speaker.setEnabled(false);
		}
//		speaker.setEnabled(false);
		addCall = (TextView) findViewById(org.linphone.R.id.addCall);
		addCall.setOnClickListener(this);
		addCall.setEnabled(false);
		transfer = (TextView) findViewById(org.linphone.R.id.transfer);
		transfer.setOnClickListener(this);
		transfer.setEnabled(false);
		options = (TextView) findViewById(org.linphone.R.id.options);
		options.setOnClickListener(this);
		options.setEnabled(false);
		pause = (TextView) findViewById(org.linphone.R.id.pause);
		pause.setOnClickListener(this);
		pause.setEnabled(false);
		hangUp = (TextView) findViewById(org.linphone.R.id.hangUp);
		hangUp.setOnClickListener(this);
		conference = (TextView) findViewById(org.linphone.R.id.conference);
		conference.setOnClickListener(this);
		dialer = (TextView) findViewById(org.linphone.R.id.dialer);
		dialer.setOnClickListener(this);
		dialer.setEnabled(false);
		numpad = (Numpad) findViewById(org.linphone.R.id.numpad);
		videoProgress =  (ProgressBar) findViewById(org.linphone.R.id.videoInProgress);
		videoProgress.setVisibility(View.GONE);
		
		
		try {
			routeLayout = (LinearLayout) findViewById(org.linphone.R.id.routesLayout);
			audioRoute = (TextView) findViewById(org.linphone.R.id.audioRoute);
			audioRoute.setOnClickListener(this);
			routeSpeaker = (TextView) findViewById(org.linphone.R.id.routeSpeaker);
			routeSpeaker.setOnClickListener(this);
			routeReceiver = (TextView) findViewById(org.linphone.R.id.routeReceiver);
			routeReceiver.setOnClickListener(this);
			routeBluetooth = (TextView) findViewById(org.linphone.R.id.routeBluetooth);
			routeBluetooth.setOnClickListener(this);
		} catch (NullPointerException npe) {
			Log.e("Bluetooth: Audio routes menu disabled on tablets for now (1)");
		}
		
		switchCamera = (ImageView) findViewById(org.linphone.R.id.switchCamera);
		switchCamera.setOnClickListener(this);
		
		mControlsLayout = (ViewGroup) findViewById(org.linphone.R.id.menu);
		
        if (!isTransferAllowed) {
        	addCall.setBackgroundResource(org.linphone.R.drawable.options_add_call);
        }

        if (!isAnimationDisabled) {
	        slideInRightToLeft = AnimationUtils.loadAnimation(this, org.linphone.R.anim.slide_in_right_to_left);
	        slideOutLeftToRight = AnimationUtils.loadAnimation(this, org.linphone.R.anim.slide_out_left_to_right);
	        slideInBottomToTop = AnimationUtils.loadAnimation(this, org.linphone.R.anim.slide_in_bottom_to_top);
	        slideInTopToBottom = AnimationUtils.loadAnimation(this, org.linphone.R.anim.slide_in_top_to_bottom);
	        slideOutBottomToTop = AnimationUtils.loadAnimation(this, org.linphone.R.anim.slide_out_bottom_to_top);
	        slideOutTopToBottom = AnimationUtils.loadAnimation(this, org.linphone.R.anim.slide_out_top_to_bottom);
        }
	...
	private void refreshInCallActions() {
		if (!LinphonePreferences.instance().isVideoEnabled()) {
			...
			if (isVideoEnabled(LinphoneManager.getLc().getCurrentCall())) {
	        	video.setBackgroundResource(org.linphone.R.drawable.video_on);
			} else {
				video.setBackgroundResource(org.linphone.R.drawable.video_off);
			}
		}
		
		try {
			if (isSpeakerEnabled) {
				speaker.setBackgroundResource(org.linphone.R.drawable.speaker_on);
				routeSpeaker.setBackgroundResource(org.linphone.R.drawable.route_speaker_on);
				routeReceiver.setBackgroundResource(org.linphone.R.drawable.route_receiver_off);
				routeBluetooth.setBackgroundResource(org.linphone.R.drawable.route_bluetooth_off);
			} else {
				speaker.setBackgroundResource(org.linphone.R.drawable.speaker_off);
				routeSpeaker.setBackgroundResource(org.linphone.R.drawable.route_speaker_off);
				if (BluetoothManager.getInstance().isUsingBluetoothAudioRoute()) {
					routeReceiver.setBackgroundResource(org.linphone.R.drawable.route_receiver_off);
					routeBluetooth.setBackgroundResource(org.linphone.R.drawable.route_bluetooth_on);
				} else {
					routeReceiver.setBackgroundResource(org.linphone.R.drawable.route_receiver_on);
					routeBluetooth.setBackgroundResource(org.linphone.R.drawable.route_bluetooth_off);
				}
			}
		} catch (NullPointerException npe) {
		...
		if (isMicMuted) {
			micro.setBackgroundResource(org.linphone.R.drawable.micro_off);
		} else {
			micro.setBackgroundResource(org.linphone.R.drawable.micro_on);
		}
		...
		if (LinphoneManager.getLc().getCallsNb() > 1) {
			...
			if (pausedCalls.size() == 1) {
				pause.setBackgroundResource(org.linphone.R.drawable.pause_on);
			} else {
				pause.setBackgroundResource(org.linphone.R.drawable.pause_off);
			}
		}
	}
	...
	private void enableAndRefreshInCallActions() {
		...
		transfer.setEnabled(getResources().getBoolean(org.linphone.R.bool.allow_transfers));
		options.setEnabled(!getResources().getBoolean(org.linphone.R.bool.disable_options_in_call) && (addCall.isEnabled() || transfer.isEnabled()));
	...
	public void onClick(View v) {
		...
		if (id == org.linphone.R.id.video) {
			enabledOrDisabledVideo(isVideoEnabled(LinphoneManager.getLc().getCurrentCall()));	
		} 
		else if (id == org.linphone.R.id.micro) {
			toggleMicro();
		} 
		else if (id == org.linphone.R.id.speaker) {
			toggleSpeaker();
		} 
		else if (id == org.linphone.R.id.addCall) {
			goBackToDialer();
		} 
		else if (id == org.linphone.R.id.pause) {
			pauseOrResumeCall();
		} 
		else if (id == org.linphone.R.id.hangUp) {
			hangUp();
		} 
		else if (id == org.linphone.R.id.dialer) {
			hideOrDisplayNumpad();
		}
		else if (id == org.linphone.R.id.conference) {
			enterConference();
		}
		else if (id == org.linphone.R.id.switchCamera) {
			if (videoCallFragment != null) {
				videoCallFragment.switchCamera();
			}
		}
		else if (id == org.linphone.R.id.transfer) {
			goBackToDialerAndDisplayTransferButton();
		}
		else if (id == org.linphone.R.id.options) {
			hideOrDisplayCallOptions();
		}
		else if (id == org.linphone.R.id.audioRoute) {
			hideOrDisplayAudioRoutes();
		}
		else if (id == org.linphone.R.id.routeBluetooth) {
		...
		else if (id == org.linphone.R.id.routeReceiver) {
		...
		else if (id == org.linphone.R.id.routeSpeaker) {
		...
		else if (id == org.linphone.R.id.callStatus) {
			LinphoneCall call = (LinphoneCall) v.getTag();
			pauseOrResumeCall(call);
		}
		else if (id == org.linphone.R.id.conferenceStatus) {
	...
	private void enabledOrDisabledVideo(final boolean isVideoEnabled) {
		...
		if (isVideoEnabled) {
			...
			if (!call.getRemoteParams().isLowBandwidthEnabled()) {
				...
				displayCustomToast(getString(org.linphone.R.string.error_low_bandwidth), Toast.LENGTH_LONG);
			}
		}
	}
	...
	public void displayCustomToast(final String message, final int duration) {
		...
		View layout = inflater.inflate(org.linphone.R.layout.toast, (ViewGroup) findViewById(org.linphone.R.id.toastRoot));

		TextView toastText = (TextView) layout.findViewById(org.linphone.R.id.toastMessage);
	...
	private void replaceFragmentVideoByAudio() {
		...
		transaction.replace(org.linphone.R.id.fragmentContainer, audioCallFragment);
		try {
...
//		Hiding controls to let displayVideoCallControlsIfHidden add them plus the callback
		...
		FragmentTransaction transaction = getSupportFragmentManager().beginTransaction();
		transaction.replace(org.linphone.R.id.fragmentContainer, videoCallFragment);
		try {
			transaction.commitAllowingStateLoss();
		} catch (Exception e) {
		}
			...
			mControlsHandler.postDelayed(mControls = new Runnable() {
				public void run() {
					...
					if (isAnimationDisabled) {
						...
						options.setBackgroundResource(org.linphone.R.drawable.options);
					} else {					
	...
	private void hideNumpad() {
		...
		dialer.setBackgroundResource(org.linphone.R.drawable.dialer_alt);
		if (isAnimationDisabled) {
			numpad.setVisibility(View.GONE);
		} else {
			Animation animation = slideOutTopToBottom;
	...
	private void hideOrDisplayNumpad() {
		...
		if (numpad.getVisibility() == View.VISIBLE) {
			...
			dialer.setBackgroundResource(org.linphone.R.drawable.dialer_alt_back);
			if (isAnimationDisabled) {
		...
		if (isTransferAllowed) {
			animation.setAnimationListener(new AnimationListener() {
				...
				public void onAnimationEnd(Animation animation) {
					...
					transfer.setVisibility(View.INVISIBLE);
					animation = AnimationUtils.loadAnimation(InCallActivity.this, org.linphone.R.anim.slide_out_top_to_bottom); // Reload animation to prevent transfer button to blink
					animation.setAnimationListener(new AnimationListener() {
						@Override
						public void onAnimationStart(Animation animation) {
						}
	...
	private void showAnimatedPortraitCallOptions() {
		...
		animation.setAnimationListener(new AnimationListener() {
			...
			@Override
			public void onAnimationEnd(Animation animation) {
				options.setBackgroundResource(org.linphone.R.drawable.options_alt);
				if (isTransferAllowed) {
	...
	protected void onDestroy() {
		...
		unbindDrawables(findViewById(org.linphone.R.id.topLayout));
		instance = null;
		super.onDestroy();
	    System.gc();
	}
	...
	private void displayConferenceHeader() {
		LinearLayout conferenceHeader = (LinearLayout) inflater.inflate(org.linphone.R.layout.conference_header, container, false);
		
		ImageView conferenceState = (ImageView) conferenceHeader.findViewById(org.linphone.R.id.conferenceStatus);
		conferenceState.setOnClickListener(this);
		if (LinphoneManager.getLc().isInConference()) {
			conferenceState.setImageResource(org.linphone.R.drawable.play);
		} else {
			conferenceState.setImageResource(org.linphone.R.drawable.pause);
		}
	...
	private void displayCall(Resources resources, LinphoneCall call, int index) {
        ...
        // Control Row
    	LinearLayout callView = (LinearLayout) inflater.inflate(org.linphone.R.layout.active_call_control_row, container, false);
		...
		// Image Row
    	LinearLayout imageView = (LinearLayout) inflater.inflate(org.linphone.R.layout.active_call_image_row, container, false);
	...
	private void setContactName(LinearLayout callView, LinphoneAddress lAddress, String sipUri, Resources resources) {
		TextView contact = (TextView) callView.findViewById(org.linphone.R.id.contactNameOrNumber);

		Contact lContact  = ContactsManager.getInstance().findContactWithAddress(callView.getContext().getContentResolver(), lAddress);
		if (lContact == null) {
	        if (resources.getBoolean(org.linphone.R.bool.only_display_username_if_unknown) && LinphoneUtils.isSipAddress(sipUri)) {
	...
	private boolean displayCallStatusIconAndReturnCallPaused(LinearLayout callView, LinphoneCall call) {
		...
		ImageView callState = (ImageView) callView.findViewById(org.linphone.R.id.callStatus);
		...
		if (call.getState() == State.Paused || call.getState() == State.PausedByRemote || call.getState() == State.Pausing) {
			callState.setImageResource(org.linphone.R.drawable.pause);
			isCallPaused = true;
			isInConference = false;
		} else if (call.getState() == State.OutgoingInit || call.getState() == State.OutgoingProgress || call.getState() == State.OutgoingRinging) {
			callState.setImageResource(org.linphone.R.drawable.call_state_ringing_default);
			isCallPaused = false;
			isInConference = false;
		} else {
			if (isConferenceRunning && call.isInConference()) {
				callState.setImageResource(org.linphone.R.drawable.remove);
				isInConference = true;
			} else {
				callState.setImageResource(org.linphone.R.drawable.play);
				isInConference = false;
			}
			isCallPaused = false;
		}
	...
	private void displayOrHideContactPicture(LinearLayout callView, Uri pictureUri, Uri thumbnailUri, boolean hide) {
		AvatarWithShadow contactPicture = (AvatarWithShadow) callView.findViewById(org.linphone.R.id.contactPicture);
		if (pictureUri != null) {
        	LinphoneUtils.setImagePictureFromUri(callView.getContext(), contactPicture.getView(), Uri.parse(pictureUri.toString()), thumbnailUri, org.linphone.R.drawable.unknown_small);
        }
...
//			backgroundResource = active ? R.drawable.cell_call_first_highlight : R.drawable.cell_call_first;
			backgroundResource = org.linphone.R.drawable.cell_call_first;
		} else {
//			backgroundResource = active ? R.drawable.cell_call_highlight : R.drawable.cell_call;
			backgroundResource = org.linphone.R.drawable.cell_call;
		}
	...
	private void registerCallDurationTimer(View v, LinphoneCall call) {
		...
		Chronometer timer = (Chronometer) v.findViewById(org.linphone.R.id.callTimer);
		if (timer == null) {
