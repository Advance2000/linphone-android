...
import org.advance2000.ui.SlidingDrawer;
...
//	private LinearLayout allAccountsLed;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, 
		...
		View view = inflater.inflate(org.linphone.R.layout.status, container, false);
		
		statusText = (TextView) view.findViewById(org.linphone.R.id.statusText);
		statusLed = (ImageView) view.findViewById(org.linphone.R.id.statusLed);
		callQuality = (ImageView) view.findViewById(org.linphone.R.id.callQuality);
		encryption = (ImageView) view.findViewById(org.linphone.R.id.encryption);
		background = (ImageView) view.findViewById(org.linphone.R.id.background);
//		allAccountsLed = (LinearLayout) view.findViewById(R.id.moreStatusLed);
		callStats = (TableLayout) view.findViewById(org.linphone.R.id.callStats);
		
		drawer = (SlidingDrawer) view.findViewById(org.linphone.R.id.statusBar);
		...
		sliderContentAccounts = (ListView) view.findViewById(org.linphone.R.id.accounts);

		voicemailCount = (TextView) view.findViewById(org.linphone.R.id.voicemailCount);
		
		exit = (TextView) view.findViewById(org.linphone.R.id.exit);
		...
		if (getResources().getBoolean(org.linphone.R.bool.exit_button_on_dialer))
		...
		mListener = new LinphoneCoreListenerBase(){
			...
			public void registrationState(final LinphoneCore lc, final LinphoneProxyConfig proxy, final LinphoneCore.RegistrationState state, String smessage) {
				...
				try {
					if (getResources().getBoolean(org.linphone.R.bool.lock_statusbar)) {
...
//						setMiniLedsForEachAccount();
			...
			public void notifyReceived(LinphoneCore lc, LinphoneEvent ev, String eventName, LinphoneContent content) {
				...
				if (unreadCount > 0) {
					voicemailCount.setText(unreadCount + " " + getResources().getString(org.linphone.R.string.voicemail_unread));
	...
	public void openOrCloseStatusBar(boolean force) {
		if (getResources().getBoolean(org.linphone.R.bool.lock_statusbar) && !force) {
			return;
		}
	...
	private void populateSliderContent() {
		if (LinphoneManager.isInstanciated() && LinphoneManager.getLc() != null) {
			...
			if (isInCall && isAttached && getResources().getBoolean(org.linphone.R.bool.display_call_stats)) {
...
//	}
	...
	private int getStatusIconResource(LinphoneCore.RegistrationState state, boolean isDefaultAccount) {
		try {
			...
			if (state == RegistrationState.RegistrationOk && defaultAccountConnected) {
				return org.linphone.R.drawable.led_connected;
			} else if (state == RegistrationState.RegistrationProgress) {
				return org.linphone.R.drawable.led_inprogress;
			} else if (state == RegistrationState.RegistrationFailed) {
				return org.linphone.R.drawable.led_error;
			} else {
				return org.linphone.R.drawable.led_disconnected;
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private String getStatusIconText(LinphoneCore.RegistrationState state) {
		...
		try {
			if (state == RegistrationState.RegistrationOk && LinphoneManager.getLcIfManagerNotDestroyedOrNull().getDefaultProxyConfig().isRegistered()) {
				return context.getString(org.linphone.R.string.status_connected);
			} else if (state == RegistrationState.RegistrationProgress) {
				return context.getString(org.linphone.R.string.status_in_progress);
			} else if (state == RegistrationState.RegistrationFailed) {
				return context.getString(org.linphone.R.string.status_error);
			} else {
				return context.getString(org.linphone.R.string.status_not_connected);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	void updateQualityOfSignalIcon(float quality) {
		if (quality >= 4) // Good Quality
		{
			callQuality.setImageResource(
					org.linphone.R.drawable.call_quality_indicator_3);
		} else if (quality >= 3) // Average quality
		{
			callQuality.setImageResource(
					org.linphone.R.drawable.call_quality_indicator_2);
		} else if (quality >= 2) // Low quality
		{
			callQuality.setImageResource(
					org.linphone.R.drawable.call_quality_indicator_1);
		...
		{
			callQuality.setImageResource(
					org.linphone.R.drawable.call_quality_indicator_0);
		}
	}
	...
	public void onResume() {
		...
		if (isInCall && (call != null || lc.getConferenceSize() > 1 || lc.getCallsNb() > 0)) {
			...
			statusLed.setImageResource(org.linphone.R.drawable.led_connected);
			statusText.setText(getString(org.linphone.R.string.status_connected));
		} else {
			...
			if (drawer != null && getResources().getBoolean(org.linphone.R.bool.lock_statusbar)) {
				drawer.lock();
			} else if (drawer != null) {
				drawer.unlock();
			}
	...
	public void refreshStatusItems(final LinphoneCall call, boolean isVideoEnabled) {
		if (call != null) {
			...
			if (mediaEncryption == MediaEncryption.SRTP || (mediaEncryption == MediaEncryption.ZRTP && call.isAuthenticationTokenVerified()) || mediaEncryption == MediaEncryption.DTLS) {
				encryption.setImageResource(org.linphone.R.drawable.security_ok);
			} else if (mediaEncryption == MediaEncryption.ZRTP && !call.isAuthenticationTokenVerified()) {
				encryption.setImageResource(org.linphone.R.drawable.security_pending);
			} else {
				encryption.setImageResource(org.linphone.R.drawable.security_ko);
			}
	...
	private void showZRTPDialog(final LinphoneCall call) {
		...
		new AlertDialog.Builder(getActivity())
	        ...
	        .setMessage(getString(org.linphone.R.string.zrtp_help))
	        .setPositiveButton(org.linphone.R.string.zrtp_accept, new DialogInterface.OnClickListener() {
	        ...
	        .setNegativeButton(org.linphone.R.string.zrtp_deny, new DialogInterface.OnClickListener() {
	            public void onClick(DialogInterface dialog, int which) { 
	            	if (call != null) {
	...
	private void initCallStatsRefresher(final LinphoneCall call, final View view) {
		...
		mTask = new TimerTask() {
			...
			public void run() {
				...
				final TextView title = (TextView) view.findViewById(org.linphone.R.id.call_stats_title);
				final TextView codec = (TextView) view.findViewById(org.linphone.R.id.codec);
				final TextView dl = (TextView) view.findViewById(org.linphone.R.id.downloadBandwith);
				final TextView ul = (TextView) view.findViewById(org.linphone.R.id.uploadBandwith);
				final TextView ice = (TextView) view.findViewById(org.linphone.R.id.ice);
				final TextView videoResolution = (TextView) view.findViewById(org.linphone.R.id.video_resolution);
				final View videoResolutionLayout = view.findViewById(org.linphone.R.id.video_resolution_layout);
	...
	class AccountsListAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			...
			if (convertView != null) {
				...
				view = LayoutInflater.from(getActivity()).inflate(org.linphone.R.layout.accounts, parent, false);
			}

			LinphoneProxyConfig lpc = (LinphoneProxyConfig) getItem(position);
			
			ImageView status = (ImageView) view.findViewById(org.linphone.R.id.State);
			
			TextView identity = (TextView) view.findViewById(org.linphone.R.id.Identity);
			...
			CheckBox isDefault = (CheckBox) view.findViewById(org.linphone.R.id.Default);
