...
import org.advance2000.compatibility.Compatibility;
import org.advance2000.setup.RemoteProvisioningLoginActivity;
import org.advance2000.setup.SetupActivity;
import org.advance2000.ui.AddressText;
import org.advance2000.LinphoneManager.AddressType;
...
public class LinphoneActivity extends FragmentActivity implements OnClickListener, ContactPicked {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		boolean useFirstLoginActivity = getResources().getBoolean(org.linphone.R.bool.display_account_wizard_at_first_start);
		...
		setContentView(org.linphone.R.layout.main);
		instance = this;
		...
		if (savedInstanceState == null) {
			if (findViewById(org.linphone.R.id.fragmentContainer) != null) {
				...
				getSupportFragmentManager().beginTransaction().add(org.linphone.R.id.fragmentContainer, dialerFragment, currentFragment.toString()).commit();
			...
			public void registrationState(LinphoneCore lc, LinphoneProxyConfig proxy, LinphoneCore.RegistrationState state, String smessage) {
				...
				if(state.equals(RegistrationState.RegistrationFailed) && newProxyConfig) {
					...
					if (proxy.getError() == Reason.BadCredentials) {
						displayCustomToast(getString(org.linphone.R.string.error_bad_credentials), Toast.LENGTH_LONG);
					}
					if (proxy.getError() == Reason.Unauthorized) {
						displayCustomToast(getString(org.linphone.R.string.error_unauthorized), Toast.LENGTH_LONG);
					}
					if (proxy.getError() == Reason.IOError) {
						displayCustomToast(getString(org.linphone.R.string.error_io_error), Toast.LENGTH_LONG);
					}
				}
			}
			...
			public void callState(LinphoneCore lc, LinphoneCall call, LinphoneCall.State state, String message) {
				if (state == State.IncomingReceived) {
					...
					if (message != null && call.getReason() == Reason.Declined) {
						displayCustomToast(getString(org.linphone.R.string.error_call_declined), Toast.LENGTH_LONG);
					} else if (message != null && call.getReason() == Reason.NotFound) {
						displayCustomToast(getString(org.linphone.R.string.error_user_not_found), Toast.LENGTH_LONG);
					} else if (message != null && call.getReason() == Reason.Media) {
						displayCustomToast(getString(org.linphone.R.string.error_incompatible_media), Toast.LENGTH_LONG);
					} else if (message != null && state == State.Error) {
						displayCustomToast(getString(org.linphone.R.string.error_unknown) + " - " + message, Toast.LENGTH_LONG);
					}
	...
	private void initButtons() {
		menu = (LinearLayout) findViewById(org.linphone.R.id.menu);
		mark = (LinearLayout) findViewById(org.linphone.R.id.mark);

		history = (RelativeLayout) findViewById(org.linphone.R.id.history);
		history.setOnClickListener(this);
		contacts = (RelativeLayout) findViewById(org.linphone.R.id.contacts);
		contacts.setOnClickListener(this);
		dialer = (ImageView) findViewById(org.linphone.R.id.dialer);
		dialer.setOnClickListener(this);
		settings = (RelativeLayout) findViewById(org.linphone.R.id.settings);
		settings.setOnClickListener(this);
		chat = (RelativeLayout) findViewById(org.linphone.R.id.chat);
		chat.setOnClickListener(this);
		aboutChat = (RelativeLayout) findViewById(org.linphone.R.id.about_chat);
		aboutSettings = (RelativeLayout) findViewById(org.linphone.R.id.about_settings);

		if (getResources().getBoolean(org.linphone.R.bool.replace_chat_by_about)) {
			chat.setVisibility(View.GONE);
			chat.setOnClickListener(null);
			findViewById(org.linphone.R.id.completeChat).setVisibility(View.GONE);
			aboutChat.setVisibility(View.VISIBLE);
			aboutChat.setOnClickListener(this);
		}
		if (getResources().getBoolean(org.linphone.R.bool.replace_settings_by_about)) {
		...
		missedCalls = (TextView) findViewById(org.linphone.R.id.missedCalls);
		missedChats = (TextView) findViewById(org.linphone.R.id.missedChats);
	}
	...
	public void hideStatusBar() {
		...
		findViewById(org.linphone.R.id.status).setVisibility(View.GONE);
		findViewById(org.linphone.R.id.fragmentContainer).setPadding(0, 0, 0, 0);
	}
	...
	public void showStatusBar() {
		...
		findViewById(org.linphone.R.id.status).setVisibility(View.VISIBLE);
		findViewById(org.linphone.R.id.fragmentContainer).setPadding(0, LinphoneUtils.pixelsToDpi(getResources(), 40), 0, 0);
	}
	...
	private void changeCurrentFragment(FragmentsAvailable newFragmentType, Bundle extras, boolean withoutAnimation) {
		...
		case HISTORY:
			if (getResources().getBoolean(org.linphone.R.bool.use_simple_history)) {
		...
		case HISTORY_DETAIL:
			...
			break;
		case CONTACTS:
			if (getResources().getBoolean(org.linphone.R.bool.use_android_native_contact_edit_interface)) {
				Intent i = new Intent();
	...
	private void updateAnimationsState() {
		isAnimationDisabled = getResources().getBoolean(org.linphone.R.bool.disable_animations) || !LinphonePreferences.instance().areAnimationsEnabled();
	}
	...
	private void changeFragment(Fragment newFragment, FragmentsAvailable newFragmentType, boolean withoutAnimation) {
		...
		if (!withoutAnimation && !isAnimationDisabled && currentFragment.shouldAnimate()) {
			if (newFragmentType.isRightOf(currentFragment)) {
				transaction.setCustomAnimations(org.linphone.R.anim.slide_in_right_to_left,
						org.linphone.R.anim.slide_out_right_to_left,
						org.linphone.R.anim.slide_in_left_to_right,
						org.linphone.R.anim.slide_out_left_to_right);
			} else {
				transaction.setCustomAnimations(org.linphone.R.anim.slide_in_left_to_right,
						org.linphone.R.anim.slide_out_left_to_right,
						org.linphone.R.anim.slide_in_right_to_left,
						org.linphone.R.anim.slide_out_right_to_left);
			}
		}
		...
		transaction.replace(org.linphone.R.id.fragmentContainer, newFragment, newFragmentType.toString());
...
//		}
		...
		LinearLayout ll = (LinearLayout) findViewById(org.linphone.R.id.fragmentContainer2);
		...
		if (newFragmentType.shouldAddItselfToTheRightOf(currentFragment)) {
			...
			transaction.addToBackStack(newFragmentType.toString());
			transaction.replace(org.linphone.R.id.fragmentContainer2, newFragment);
		} else {
			if (newFragmentType == FragmentsAvailable.DIALER
			...
			if (!withoutAnimation && !isAnimationDisabled && currentFragment.shouldAnimate()) {
				if (newFragmentType.isRightOf(currentFragment)) {
					transaction.setCustomAnimations(org.linphone.R.anim.slide_in_right_to_left, org.linphone.R.anim.slide_out_right_to_left, org.linphone.R.anim.slide_in_left_to_right, org.linphone.R.anim.slide_out_left_to_right);
				} else {
					transaction.setCustomAnimations(org.linphone.R.anim.slide_in_left_to_right, org.linphone.R.anim.slide_out_left_to_right, org.linphone.R.anim.slide_in_right_to_left, org.linphone.R.anim.slide_out_right_to_left);
				}
			}

			transaction.replace(org.linphone.R.id.fragmentContainer, newFragment);
		}
		transaction.commitAllowingStateLoss();
		getSupportFragmentManager().executePendingTransactions();
	...
	public void displayHistoryDetail(String sipUri, LinphoneCallLog log) {
		...
		Fragment fragment2 = getSupportFragmentManager().findFragmentById(org.linphone.R.id.fragmentContainer2);
	...
	public void onClick(View v) {
		...
		} else if (id == org.linphone.R.id.settings) {
			changeCurrentFragment(FragmentsAvailable.SETTINGS, null);
			settings.setSelected(true);
		} else if (id == org.linphone.R.id.about_chat) {
			Bundle b = new Bundle();
		...
		} else if (id == org.linphone.R.id.about_settings) {
			Bundle b = new Bundle();
		...
		} else if (id == org.linphone.R.id.chat) {
			changeCurrentFragment(FragmentsAvailable.CHATLIST, null);
	...
	private void displayMissedCalls(final int missedCallsCount) {
		if (missedCallsCount > 0) {
			...
			if (!isAnimationDisabled) {
				missedCalls.startAnimation(AnimationUtils.loadAnimation(LinphoneActivity.this, org.linphone.R.anim.bounce));
			}
		} else {
	...
	private void displayMissedChats(final int missedChatCount) {
		if (missedChatCount > 0) {
			...
			if (!isAnimationDisabled) {
				missedChats.startAnimation(AnimationUtils.loadAnimation(LinphoneActivity.this, org.linphone.R.anim.bounce));
			}
		} else {
...
//		changeCurrentFragment(FragmentsAvailable.DIALER, extras);
	...
	public void sendLogs(Context context, String info){
		final String appName = context.getString(org.linphone.R.string.app_name);

		Intent i = new Intent(Intent.ACTION_SEND);
		i.putExtra(Intent.EXTRA_EMAIL, new String[]{ context.getString(org.linphone.R.string.about_bugreport_email) });
		...
		if (extras != null && extras.getBoolean("GoToChat", false)) {
			...
			if (dialerFragment != null) {
				if (extras != null && extras.containsKey("SipUriOrNumber")) {
					if (getResources().getBoolean(org.linphone.R.bool.automatically_start_intercepted_outgoing_gsm_call)) {
