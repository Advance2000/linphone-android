...
public class ContactsManager {
	...
	public void initializeSyncAccount(Context context, ContentResolver contentResolver) {
		...
		if(accounts != null && accounts.length == 0) {
			Account newAccount = new Account(context.getString(org.linphone.R.string.sync_account_name), context.getPackageName());
			try {
			...
			public void run() {
				if(sipContactCursor != null && sipContactCursor.getCount() > 0) {
					for (int i = 0; i < sipContactCursor.getCount(); i++) {
						...
						if (LinphoneActivity.isInstanciated() && LinphoneActivity.instance().getResources().getBoolean(org.linphone.R.bool.use_linphone_tag)) {
