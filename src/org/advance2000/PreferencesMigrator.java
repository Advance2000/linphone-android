...
import org.advance2000.LinphonePreferences.AccountBuilder;
...
public class PreferencesMigrator {
	...
	public boolean isMigrationNeeded() {
		int accountNumber = mOldPrefs.getInt(getString(org.linphone.R.string.pref_extra_accounts), -1);
	...
	public void doMigration() {
		...
		mNewPrefs.setFrontCamAsDefault(getPrefBoolean(org.linphone.R.string.pref_video_use_front_camera_key, true));
		mNewPrefs.setWifiOnlyEnabled(getPrefBoolean(org.linphone.R.string.pref_wifi_only_key, false));
		mNewPrefs.useRandomPort(getPrefBoolean(org.linphone.R.string.pref_transport_use_random_ports_key, true), false);
		mNewPrefs.setPushNotificationEnabled(getPrefBoolean(org.linphone.R.string.pref_push_notification_key, false));
		mNewPrefs.setPushNotificationRegistrationID(getPrefString(org.linphone.R.string.push_reg_id_key, null));
		mNewPrefs.setDebugEnabled(getPrefBoolean(org.linphone.R.string.pref_debug_key, false));
		mNewPrefs.setBackgroundModeEnabled(getPrefBoolean(org.linphone.R.string.pref_background_mode_key, true));
		mNewPrefs.setAnimationsEnabled(getPrefBoolean(org.linphone.R.string.pref_animation_enable_key, false));
		mNewPrefs.setAutoStart(getPrefBoolean(org.linphone.R.string.pref_autostart_key, false));
		mNewPrefs.setSharingPictureServerUrl(getPrefString(org.linphone.R.string.pref_image_sharing_server_key, null));
		mNewPrefs.setRemoteProvisioningUrl(getPrefString(org.linphone.R.string.pref_remote_provisioning_key, null));
	...
	private void doAccountsMigration() {
		...
		for (int i = 0; i < mOldPrefs.getInt(getString(org.linphone.R.string.pref_extra_accounts), 1); i++) {
			doAccountMigration(i, i == getPrefInt(org.linphone.R.string.pref_default_account_key, 0));
		}
	}
	...
	private void doAccountMigration(int index, boolean isDefaultAccount) {
		...
		String username = getPrefString(getString(org.linphone.R.string.pref_username_key) + key, null);
		String userid = getPrefString(getString(org.linphone.R.string.pref_auth_userid_key) + key, null);
		String password = getPrefString(getString(org.linphone.R.string.pref_passwd_key) + key, null);
		String domain = getPrefString(getString(org.linphone.R.string.pref_domain_key) + key, null);
		if (username != null && username.length() > 0 && password != null) {
			String proxy = getPrefString(getString(org.linphone.R.string.pref_proxy_key) + key, null);
			String expire = getPrefString(org.linphone.R.string.pref_expire_key, null);
			...
			if (getPrefBoolean(getString(org.linphone.R.string.pref_enable_outbound_proxy_key) + key, false)) {
				builder.setOutboundProxyEnabled(true);
			}
			if (mResources.getBoolean(org.linphone.R.bool.enable_push_id)) {
				String regId = mNewPrefs.getPushNotificationRegistrationID();
				String appId = getString(org.linphone.R.string.push_sender_id);
